...
| `oob` | _[OOBReporting](#oobreporting)_ |  false  |  | OOB enables out-of-band ORCA load reporting. When set, Envoy opens a<br />server-streaming gRPC connection to each endpoint's<br />xds.service.orca.v3.OpenRcaService/StreamCoreMetrics and pulls load<br />reports periodically, instead of relying solely on in-band ORCA metrics<br />carried in response headers/trailers. In-band metrics (and their header<br />stripping via KeepResponseHeaders) continue to work alongside OOB.<br />The backend must implement OpenRcaService for this to take effect.<br />An empty block (\{\}) enables OOB with Envoy's defaults. |
...
BackendUtilization load balancer. The presence of this block enables OOB
reporting (Envoy's enable_oob_load_report).
...
| Field | Type | Required | Default | Description |
| ---   | ---  | ---      | ---     | ---         |
| `reportingPeriod` | _[Duration](https://gateway-api.sigs.k8s.io/reference/api-spec/1.5/spec/#duration)_ |  false  |  | ReportingPeriod is how often Envoy requests load reports from the server.<br />The server may report less frequently than requested. Defaults to Envoy's<br />default of 10s. |
...
| `authority` | _string_ |  false  |  | Authority overrides the :authority header on the OOB gRPC stream.<br />Must not contain NUL, CR, or LF characters (matching Envoy's constraint on<br />the OrcaOobReportingConfig authority field), which also guards against<br />header injection. |
