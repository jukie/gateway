...
func TestGetBTPRoutingTypeForRoute(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "listener-only BTP with no gateway fallback, route on different listener returns nil",
			btps: []*egv1a1.BackendTrafficPolicy{
				{
					ObjectMeta: metav1.ObjectMeta{
						Namespace: "default",
						Name:      "btp-listener-https",
					},
					Spec: egv1a1.BackendTrafficPolicySpec{
						PolicyTargetReferences: egv1a1.PolicyTargetReferences{
							TargetRef: &gwapiv1.LocalPolicyTargetReferenceWithSectionName{
		...
		{
			name: "route with nil sectionName does not match listener-level BTP",
			btps: []*egv1a1.BackendTrafficPolicy{
				{
					ObjectMeta: metav1.ObjectMeta{
						Namespace: "default",
		...
		{
			name: "multiple BTPs targeting different listeners, correct one matches",
			btps: []*egv1a1.BackendTrafficPolicy{
				{
					ObjectMeta: metav1.ObjectMeta{
						Namespace: "default",
						Name:      "btp-listener-http",
					},
					Spec: egv1a1.BackendTrafficPolicySpec{
						PolicyTargetReferences: egv1a1.PolicyTargetReferences{
							TargetRef: &gwapiv1.LocalPolicyTargetReferenceWithSectionName{
			...
			routeKind:    gwapiv1.Kind("HTTPRoute"),
			gatewayNN:    types.NamespacedName{Namespace: "default", Name: "gateway-1"},
			listenerName: ptr.To(gwapiv1.SectionName("https")),
			expected:     &endpointRouting,
		},
		{
			name: "listener BTP via targetRefs (plural) with sectionName",
			btps: []*egv1a1.BackendTrafficPolicy{
				{
					ObjectMeta: metav1.ObjectMeta{
						Namespace: "default",
						Name:      "btp-multi-listener",
					},
					Spec: egv1a1.BackendTrafficPolicySpec{
						PolicyTargetReferences: egv1a1.PolicyTargetReferences{
							TargetRefs: []gwapiv1.LocalPolicyTargetReferenceWithSectionName{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := GetBTPRoutingTypeForRoute(tt.btps, tt.routeNN, tt.routeKind, tt.gatewayNN, tt.listenerName)
